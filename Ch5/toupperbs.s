...
store_fd_in:
movl $FILDES, %ebx
movl %eax, ST_FD_IN(%ebx)	#store file descriptor
...
store_fd_out:
movl $FILDES, %ebx
movl %eax, ST_FD_OUT(%ebx)	#store file descriptor

read_loop_begin:
movl $SYS_READ, %eax		#read in a block from input file
				#movl 3 %eax
movl $FILDES, %ebx
movl ST_FD_IN(%ebx), %ebx	#get input file descriptor
...
movl $FILDES, %ebx
movl ST_FD_OUT(%ebx), %ebx	#the filedescriptor of the output file
...
movl $SYS_CLOSE, %eax
movl $FILDES, %ebx
movl ST_FD_OUT(%ebx), %ebx	#file descriptor output file
int $LINUX_SYSCALL

movl $SYS_CLOSE, %eax		#we do it again since %eax could be
movl $FILDES, %ebx				#modified by our syscall
movl ST_FD_IN(%ebx), %ebx	#file desciptor input file
int $LINUX_SYSCALL
...
# To run on a 64 bit linux machine use the following: 

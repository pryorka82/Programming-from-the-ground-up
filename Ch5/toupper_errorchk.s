...
#          converted to uppercase. It also has 
#	   error checking after each system call.
#PROCESSING:
...
cmpl $END_OF_FILE, %eax   #if found or on error, go to the end
jle end_loop
continue_read_loop:
...
movl %eax, %edx            #size of buffer loaded to %edx
movl $SYS_WRITE, %eax
...
write_error:
movl $errmsg, %ecx		#error message loaded in %ecx to be printed
movl $SYS_WRITE, %eax           #set %eax to 4
movl $STDOUT, %ebx      #the filedescriptor of the output file
movl $17, %edx		#error codes can be 4 characters in length

...
cmpl $1, %eax	#If the number is 1, that is our base case, and we simply
		#return (1 is already in %eax as the return value
je end_factorial
decl %eax	#otherwise, decrease the value
pushl %eax	#push it for our call to factorial
call factorial	#call factorial
movl 8(%ebp), %ebx #%eax has the returen value, so we relaod our parameter
		   #into %ebx
imull %ebx, %eax   #multiply that by the result of the last call to factorial
		   #(in %eax) the answer is stored in %eax, which is good
